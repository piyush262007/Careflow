...
export const PatientSidebar: React.FC<PatientSidebarProps> = ({
  ...
  const navItems = [
    ...
    { id: 'appointments', label: 'Appointments', icon: Calendar, badge: '1' },
    { id: 'doctors', label: 'Find a Doctor', icon: UserCheck },
    { id: 'records', label: 'Health Records', icon: FileText },
    { id: 'prescriptions', label: 'Prescriptions', icon: Pill, badge: '2' },
    { id: 'queue', label: 'Queue Status', icon: Clock, badge: '#2' },
      ...
      <div className="space-y-4 pt-4 border-t border-[var(--border-subtle)]">
        ...
        <div className="p-3 rounded-2xl bg-[var(--bg-card-bg)] border border-[var(--border-color)] flex items-center justify-between gap-2">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="h-9 w-9 rounded-xl bg-gradient-to-tr from-emerald-600 to-teal-400 text-white flex items-center justify-center font-bold text-sm shadow-sm shrink-0">
              {currentUser?.name?.charAt(0) || 'P'}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-xs font-bold text-[var(--text-primary)] truncate">{currentUser?.name || 'Patient'}</p>
