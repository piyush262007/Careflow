...
export const DoctorSidebar: React.FC<DoctorSidebarProps> = ({
  ...
  const navItems = [
    ...
    { id: 'appointments', label: "Today's Appointments", icon: Calendar, badge: '12' },
    { id: 'queue', label: 'Patient Queue', icon: Clock, badge: '4 Live' },
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div className="flex flex-col">
              <span className="font-heading font-extrabold text-base tracking-tight text-[var(--text-primary)] flex items-center gap-1.5">
                CareFlow
                <span className="rounded-full bg-blue-500/10 px-1.5 py-0.5 text-[9px] font-bold text-blue-600 dark:text-blue-400 border border-blue-500/20">
                  CLINICAL
                </span>
              </span>
            </div>
      ...
      <div className="space-y-4 pt-4 border-t border-[var(--border-subtle)]">
        ...
        <div className="p-3 rounded-2xl bg-[var(--bg-card-bg)] border border-[var(--border-color)] flex items-center justify-between gap-2">
          <div className="flex items-center gap-2.5 min-w-0">
            <div className="h-9 w-9 rounded-xl bg-gradient-to-tr from-blue-600 to-indigo-500 text-white flex items-center justify-center font-bold text-sm shadow-sm shrink-0">
              {currentUser?.name?.charAt(4) || 'D'}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-xs font-bold text-[var(--text-primary)] truncate">{currentUser?.name || 'Dr. Sarah Chen'}</p>
              <p className="text-[10px] text-[var(--text-muted)] truncate">{currentUser?.email || 'doctor@careflow.demo'}</p>
            </div>
          </div>
